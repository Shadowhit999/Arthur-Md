...
smd({ cmdname: "updatenow", type: "owner", info: "Downloads the entire Git repository from https://github.com/STAR-KING0/Arthur-Md.git on the root of the app.", fromMe: s_ser, filename: __filename }, async (message) => {
  try {
    const repoUrl = 'https://github.com/STAR-KING0/Arthur-Md.git';
  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`ARTHUR MD UPDATE WAS SUCCESSFULL ✅`");
      } else {
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `𝗔𝗥𝗧𝗛𝗨𝗥-𝗠𝗗  2.0.0 𝛲𝛥𝑇𝐶𝛨\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\n 𝗔𝗥𝗧𝗛𝗨𝗥-𝗠𝗗  `;
    ...
    try {
      ...
      if (!_0x5eb566) {
        return await _0x15cc76.reply(
          "*Please Give Me Text. Example: _" + prefix + "trt en i love Arthur_*"
        );
      }
